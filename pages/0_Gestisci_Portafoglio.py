"""
Aggiungi, modifica o elimina azioni / ETF / obbligazioni / fondi / liquidità
direttamente dall'app, senza toccare file o codice.
"""
...
with st.expander("Come compilare i campi", expanded=False):
    st.markdown(
        ...
        "- **Obbligazioni e Fondi/SICAV**: se non trovi un ticker su Yahoo Finance (capita "
        "spesso per singoli BTP/ISIN o fondi non quotati), scrivi una sigla a tua scelta in "
        "`ticker` (es. `BTP-2030`) e compila `manual_price` con il prezzo/NAV corrente — "
        "dovrai aggiornarlo tu di tanto in tanto.\n"
        "- **Liquidità**: metti in `ticker` un'etichetta (es. `Conto Deposito XYZ`), in "
        "`quantity` l'importo in euro, e lascia vuoti `buy_price`/`manual_price` — viene "
        "trattata sempre a valore nominale, senza P&L.\n"
        "- **quantity**: quantita' posseduta (numero di azioni/quote, nominale per le "
        "obbligazioni, importo in euro per la liquidità).\n"
        "- **buy_price**: prezzo medio di carico (non serve per la liquidità).\n"
        "- **category**: usata per i grafici di allocazione e per il ribilanciamento.\n"
...
edited = st.data_editor(
    ...
    column_config={
        ...
        "quantity": st.column_config.NumberColumn("Quantità / Importo", required=True, min_value=0.0),
        "buy_price": st.column_config.NumberColumn("Prezzo di carico", min_value=0.0),
