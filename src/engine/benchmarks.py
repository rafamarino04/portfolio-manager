...
def random_entry_monte_carlo(histories: dict[str, pd.DataFrame], n_trades_target: int,
                              ...
                              target_atr_mult: float = 4.0) -> RandomEntryResult:
    ...
    **da dove viene il segnale**."""
    ...
    for symbol, hist in histories.items():
        ...
        if len(h) > 5:
            frames[symbol] = h
    if not frames or n_trades_target <= 0:
    ...
    for _ in range(runs):
        ...
        for _ in range(n_trades_target):
            ...
            if atr_series is None or h.shape[0] < 3:
                continue
            # L'ingresso non può cadere sull'ultimo bar: serve almeno un
            # bar successivo su cui eseguire il fill al next-open.
            i = int(rng.integers(0, len(h) - 2))
            entry_date = h.index[i + 1]
            entry_price = float(h["Open"].iloc[i + 1])

            atr_val = _atr_at(atr_series, h.index[i])
            ...
            entry_cost = costs.entry_cost_eur(sizing.notional_eur, currencies.get(symbol))
            ledger.open_position(OpenPosition(
                ...
                currency=currencies.get(symbol), signal_date=h.index[i],
            ))

            # Percorre i bar successivi con le stesse regole di uscita.
            exit_done = False
            for j in range(i + 2, len(h)):
                bar = h.iloc[j]
                ...
                if event is not None:
                    ...
                    ledger.close_position(symbol, h.index[j], event.price, event.reason,
                                           costs.exit_cost_eur(notional, currencies.get(symbol)),
                                           gapped=event.gapped)
                    exit_done = True
                    break
            if not exit_done:
                ...
                ledger.close_position(symbol, h.index[-1], price, "chiusura_forzata",
                                       costs.exit_cost_eur(abs(sizing.size * price),
...
def _atr_at(atr_series: pd.Series, when) -> float | None:
    try:
        idx = [ts.date() if hasattr(ts, "date") else ts for ts in atr_series.index]
        lookup = dict(zip(idx, atr_series.values))
        val = lookup.get(when)
